...
class Pivoter {
 protected:
  /**
   * This class describes the pivoting ball, which rolls on points and links the
   * mesh.
   */
  class Edge {
   protected:
    ...
    /** index of the opposite vertice, it and _id_vertices form the last
     * triangle */
    uint32_t _id_opposite;
    /** center point of the last ball, it may not be one in the cloud */
    PointNT _center;
    /** whether this ball it to the it rolled in the back face of surface (not
     * normal direction) */
    bool _is_back_ball;
   ...
   public:
    ...
    Edge(const std::vector<uint32_t> &edge, const uint32_t id_opposite,
         const PointNT &center, const bool is_back_ball = false);
     ...
     */
    void setIdVertice(const size_t id, const uint32_t id_vertice);
    ...
    /**
     * get the reverse signature of this edge, [index end vertice, index start
     * vertice]
     * @return
     */
  ...
  /**
   * Front manages the edges, which ones are to be pivoted, which ones are
   * pivoted.
   */
  class Front {
   ...
   protected:
    ...
    /** The set of edges which are pivoted, but cannot reach suitable points.
     * Edges here should be open boundary */
    std::map<Signature, Edge> _boundary;
    /** The set of successfully pivoted edges */
    std::set<Signature> _finished;
    /** The edge that is being pivoted and waiting for pivoting result: boundary
     * or pivoted? */
   ...
   public:
     ...
     * @return
     */
    typename Edge::Ptr getActiveEdge();
     ...
     */
    void addTriangle(const pcl::Vertices::ConstPtr &seed, const PointNT &center,
                     const bool is_back_ball);

    /**
     * extend the edges with one pivoted edge and vertice on new ball. the
     * vertice on new ball should form two edges with the start vertice and end
     * vertice of the old edge
     * @param last_edge the edge which was pivoted and contributes two vertices
     * to new edges
     ...
     */
    void addPoint(const Edge &last_edge, const uint32_t id_vetice_extended,
     ...
     * @return
     */
    bool isEdgeOnFront(const Edge &edge) const;

    /**
     * removes the edge with signature [id0,id1] or [id1,id0] in edges to
     * pivoted
     * @param id0
     * @param id1
     */
     ...
     * @param is_boundary
     */
    void setFeedback(const bool is_boundary);
     ...
     * @return
     */
    bool isEdgeFinished(const Edge &edge) const;
 ...
 protected:
  ...
  /** edge manager */
  Front _front;
  /** search radius. default value is -1, radius would be guessed if it is
   * non-positive */
  double _radius;
  /** whether balls on the back surface would be considered */
  bool _is_allow_back_ball;
  /** whether the pivoting ball could flip from front face to back face or vice
   * versa */
  bool _is_allow_flip;
   ...
   */
  bool findSeed(pcl::Vertices::Ptr &seed, PointNT &center, bool &is_back_ball);

  /**
   * pivoting around edge
   * @param edge edge for pivoting
   * @param id_extended index of the hitting vertice, which would bring more
   * edges for pivoting
   * @param center_new
   * @param is_back_ball
   * @return whether pivoting is successful, if not, it is a bounday
   */
  bool pivot(const Edge &edge, uint32_t &id_extended, PointNT &center_new,
   ...
   * @param mesh
   */
  void proceedFront(pcl::PolygonMesh::Ptr &mesh);

  /**
   * find the center of newly pivoted ball, if empty, then pivoting is not
   * successful
   * @param is_back_first whether to consider assumed center on back surface
   * first. it changes the result if flipping between front surface and back
   * surface is enabled (setAllowFlip)
   ...
   * @return
   */
  std::shared_ptr<PointNT> getBallCenter(const bool is_back_first,
 ...
 public:
   ...
   * @param radius
   */
  void setSearchRadius(const double radius) { _radius = radius; }

  /**
   * get the radius of ball
   * @return
   */
  double getSearchRadius() const { return _radius; }

  /**
   * set whether ball on the back surface is allowed. if this value is set to
   * true, the ball only pivots on the direction of the normal vectors
   * @param is_allow_back_ball
   */
  void setAllowBackBall(const bool is_allow_back_ball) {
   ...
   * @return
   */
  bool getAllowBackBall() const { return _is_allow_back_ball; }

  /**
   * set whether flipping between front surface and back surface is allow. if
   * not, the ball would only pivot so it stays on one side of the surface,
   * depending on where the center of seed ball was found
   * @param is_allow_flip
   */
  void setAllowFlip(const bool is_allow_flip) {
   ...
   * @return
   */
  bool getAllowFlip() const { return _is_allow_flip; }

  /**
   * this function starts the preparation, pivoting process and generates the
   * final mesh
   * @param cloud
   * @return
   */
  pcl::PolygonMesh::Ptr proceed();

  /**
   * set the cloud to reconstruct
   * @param cloud
   */
  void setInputCloud(const typename pcl::PointCloud<PointNT>::ConstPtr &cloud);

  /**
   * estimate one radius and set it as ball radius. num_sample_point points
   * would be selected randomly in cloud, as least ratio_success of the sample
   * points must have at least num_point_in_radius neighbors within the
   * estimated radius.
   * @param num_sample_point
   * @param num_point_in_radius
   * @param ratio_success
   */
  void setEstimatedRadius(const int num_sample_point,
